...
class train:
	...
	def start(self, train_x, train_y, val_x, val_y):
		for epoch in range(self.nn.epochs):
			iters = int( len(train_x) / self.nn.batch_size )
			for iter_ in range(iters):
				batch_x = train_x[iter_*self.nn.batch_size:(iter_+1)*self.nn.batch_size, :]
				batch_y = train_y[iter_*self.nn.batch_size:(iter_+1)*self.nn.batch_size, :]
	...
	def save_models(self):
		...
		print("session saved in {0}".format(path))