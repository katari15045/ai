...
class pre_processor:
	...
	def get_data(self, testing=False):
		...
		# each sample is converted from 2d to 1d for MinMaxScaler
		if(testing == False):
		...
		# each sample is converted from 2d to 3d
		if(testing == False):
			train_x = np.reshape(train_x, (train_x.shape[0], img_rows, img_cols, 1))
		else:
			test_x = np.reshape(test_x, (test_x.shape[0], img_rows, img_cols, 1))
